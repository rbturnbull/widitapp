...
Override ``loss()`` with ``@method`` to configure the loss in a child app.
Cluey forwards its arguments from ``train()`` and exposes them as CLI options.
Without an override, regression uses MSE (or ``loss_fn="smoothl1"``), and
diffusion retains its original noise-MSE plus variance objective.

For example, add a penalty for mismatched mean image intensity:
...
.. code-block:: python
    ...
    class ImageLoss(torch.nn.Module):
        def __init__(self, mean_weight, use_diffusion):
            super().__init__()
        ...
        def forward(self, prediction, target):
            ...
            loss = self.mean_weight * (
                prediction.mean(dims) - target.mean(dims)
            ).square().mean()
            if not self.use_diffusion:
                loss = loss + (prediction - target).square().mean()
            return loss

    class MyApp(WiDiTApp):
        # Also implement datasets() for your application.
        @method
        def loss(self, mean_weight: float = 0.1, use_diffusion: bool = True):
            return ImageLoss(mean_weight, use_diffusion)

Call ``MyApp().train(mean_weight=0.2, use_diffusion=False, ...)`` or use
``--mean-weight 0.2`` on the train command.

In regression, the returned module defines the entire objective and must return
a scalar. In diffusion, it receives the unclipped clean-image estimate and clean
target, and its result is added to the standard objective. Return a scalar batch
mean or a tensor of shape ``[N]``; include component weights inside your module.
The estimate is computed from the sampled noisy target in the existing forward
pass, with gradients retained. It is not an image generated by a full sampling
loop. Estimates at high noise levels can have large errors, so choose weights
accordingly. Loss modules are moved to the training device; their parameters are
not optimized by the model optimizer.

The base ``loss()`` returns ``None`` for diffusion, adding no penalty. With this
default, validation still measures noise MSE only. With an extra diffusion loss,
validation and best-checkpoint selection use the full objective, including the
variance term and the extra image loss. Regression validation uses the selected
loss and also reports MSE and Smooth L1.
